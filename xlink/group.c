...
static	SMemoryPool* createpool(void)
{
	...
	if((pool=(SMemoryPool* )malloc(sizeof(SMemoryPool)))!=NULL)
	{
...
BOOL	alloc_abs_from_pool(SMemoryPool* pool, ULONG size, ULONG org)
{
	...
	ppchunk=&pool->pFreeChunks;

	while(*ppchunk)
	{
		if( org>=(*ppchunk)->Org
		&&	org+size<=(*ppchunk)->Org+(*ppchunk)->Size )
		{
			SMemChunk* newchunk;

			if((newchunk=(SMemChunk* )malloc(sizeof(SMemChunk)))!=NULL)
			{
				newchunk->pNext=(*ppchunk)->pNext;
				(*ppchunk)->pNext=newchunk;

				newchunk->Org=org+size;
				newchunk->Size=(*ppchunk)->Org+(*ppchunk)->Size-(org+size);

				(*ppchunk)->Size=org-(*ppchunk)->Org;
...
void	group_Alloc(SSection* sect)
{
	...
	while(group)
	{
		if(strcmp(group->Name,groupname)==0)
		{
			...
			for(i=0; i<group->TotalPools; i+=1)
			{
				SMemoryPool* pool;
				...
				||	sect->Bank==pool->BankId )
				{
					...
					{
						if(alloc_abs_from_pool(pool,sect->Size,sect->Org))
						{
							sect->Bank=pool->BankId;
							sect->Assigned=TRUE;
							return;
...
void	group_SetupGameboy(void)
{
	...
	for(i=0; i<256; i+=1)
	{
		...
		if(i==0)
		{
			codepools[i]->AddressingOffset=0x0000;
		}
		else
		{
			codepools[i]->AddressingOffset=0x4000;
		}
	}
