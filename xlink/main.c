...
printUsage(void) {
	printf("xlink (ASMotor v" ASMOTOR_VERSION ")\n"
	       ...
	       "    -h  This text\n"
	       "\n"
	       "    -a<file>  Machine definition file\n"
	       "\n"
	       "    -c<config>  Machine definition\n"
	       "          -camiga     Amiga\n"
	       "          -ccbm64     Commodore 64\n"
	       ...
	       "          -chc8b      HC800 16 KiB ROM\n"
	       "          -chc8s      HC800 small mode (64 KiB text + data + bss)\n"
	       "          -chc8sh     HC800 small Harvard mode (64 KiB text + data, 64 KiB data + bss)\n"
	       "          -chc8m      HC800 medium mode (32 KiB text + data + bss, 32 KiB sized\n"
	       "                      banks text)\n"
	       "          -chc8mh     HC800 medium Harvard executable (32 KiB text + 32 KiB\n"
	       "                      sized text banks, 64 KiB data + bss)\n"
	       "          -chc8l      HC800 large mode (32 KiB text + data + bss, 32 KiB sized\n"
	       "                      banks text + data + bss)\n"
...
handleLegacyTargetOption(const string* target) {
	...
	} else if (str_EqualConst(target,
	                          "hc8mh")) { /* HC800, CODE: 32 KiB text, CODE: 32 KiB sized text banks, DATA: 64 KiB data + bss */
		group_SetupHC8XXMediumHarvard();
		g_outputFormat = FILE_FORMAT_HC800;
		g_allowedFormats = FF_HC800;
		g_hc800Config = hc800_ConfigMediumHarvard;
	} else if (str_EqualConst(target,
	                          "hc8l")) { /* HC800, CODE: 32 KiB text + data + bss, CODE: 32 KiB sized banks text + data + bss */
...
main(int argc, char* argv[]) {
	...
	patch_Process(format_SupportsReloc(g_outputFormat), format_SupportsOnlySectionRelativeReloc(g_outputFormat),
