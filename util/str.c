...
static void copyOnWrite(string** str) {
	if ((*str)->refCount != 1) {
...
INLINE string* str_Alloc(size_t length) {
...
string* str_CreateLength(const char* data, size_t length) {
...
string* str_Slice(const string* str1, ssize_t index, size_t length) {
	if (index < 0)
...
bool str_EqualConst(const string* str1, const char* str2) {
	size_t length1 = str_Length(str1);
	char ch2;
...
string* str_Replace(const string* str, char search, char replace) {
...
void str_ToUpperReplace(string** str) {
	copyOnWrite(str);
