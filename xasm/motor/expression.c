...
#define COMBINE_BITWISE(NAME, OP, TOKEN) COMBINE(NAME, (uint32_t) left->value.integer OP(uint32_t) right->value.integer, TOKEN)

#define COMBINE_ARITHMETIC(NAME, OP, TOKEN) COMBINE(NAME, left->value.integer OP right->value.integer, TOKEN)

#define CREATE_BINARY_FUNC(NAME, OP, TOKEN) COMBINE(NAME, OP(left->value.integer, right->value.integer), TOKEN)
...
COMBINE_ARITHMETIC(Mul, *, T_OP_MULTIPLY)
