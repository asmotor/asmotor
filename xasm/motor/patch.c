...
reduceOperation(const SPatch* patch, SExpression* expression, int32_t* result) {
	switch (expression->operation) {
		...
		case T_FUNC_BANK: {
			...
			if (expression->value.symbol->type == SYM_LABEL && (expression->value.symbol->section->flags & SECTF_BANKFIXED)) {
				*result = expression->value.symbol->section->bank;
				expr_Clear(expression);
...
static bool
reduceRiscvOperation(const SPatch* patch, SExpression* expression, int32_t* result) {
	if (!reduceExpression(patch, expression->right, result))
		return false;

	expr_Free(expression->left);

	SExpression* opcode_expr = expression->right;
	*expression = *opcode_expr;
	mem_Free(opcode_expr);
...
reduceExpression(const SPatch* patch, SExpression* expression, int32_t* result) {
	...
	switch (expr_Type(expression)) {
		...
		case EXPR_RISCV_ELF_RELOC:
			return reduceRiscvOperation(patch, expression, result);
		default:
