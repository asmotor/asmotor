...
static bool verifyPointers(SExpression* left, SExpression* right) {
...
static bool verifyPointer(SExpression* expression) {
	if (expression != NULL)
		return true;
...
static SExpression* mergeExpressions(SExpression* left, SExpression* right, EToken operation) {
	SExpression* expr;
...
static bool isSymbol(SExpression* expression) {
...
SExpression* expr_PcRelative(SExpression* pExpr, int adjustment) {
	if (!verifyPointer(pExpr))
		return NULL;

	if (expr_IsConstant(pExpr) && (g_pCurrentSection->Flags & (SECTF_LOADFIXED | SECTF_ORGFIXED))) {
		pExpr->value.integer -= (g_pCurrentSection->PC + g_pCurrentSection->BasePC + g_pCurrentSection->OrgOffset - adjustment);
		return pExpr;
	} else if (g_pCurrentSection->Flags & (SECTF_LOADFIXED | SECTF_ORGFIXED)) {
		return expr_Add(
            pExpr,
            expr_Const(adjustment - (g_pCurrentSection->PC + g_pCurrentSection->BasePC + g_pCurrentSection->OrgOffset))
        );
	} else {
		SExpression* r = (SExpression*) mem_Alloc(sizeof(SExpression));
