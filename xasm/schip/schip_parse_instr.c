...
#define MODE_REG    0x01u
#define MODE_IMM    0x02u
#define MODE_I      0x04u
#define MODE_DT     0x08u
#define MODE_ST     0x10u
#define MODE_I_IND  0x20u
#define MODE_IMM_V0 0x40u
#define MODE_RPL    0x80u

#define REGISTER_NONE 0xFFu
...
handleDRW(SAddressMode* mode1, SAddressMode* mode2, SAddressMode* mode3, uint16_t opcode) {
	sect_OutputExpr16(expr_Or(expr_Const(opcode | (mode1->registerIndex << 8) | (mode2->registerIndex << 4)),
	                          expr_And(expr_CheckRange(mode3->expression, 0, 16), expr_Const(15))));
	return true;
}
...
handleModeRegImm(SAddressMode* mode1, SAddressMode* mode2, SAddressMode* mode3, uint16_t opcode) {
	sect_OutputExpr16(expr_Or(expr_Const(opcode | (mode1->registerIndex << 8)),
	                          expr_And(expr_CheckRange(mode2->expression, -128, 255), expr_Const(255))));
	return true;
}
