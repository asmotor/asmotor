...
static SInstruction g_integerInstructions[T_68K_INTEGER_LAST - T_68K_INTEGER_FIRST + 1];
...
static bool
handleArithmetic(ESize size, SAddressingMode* src, SAddressingMode* dest, ETargetToken address, ETargetToken immediate,
                 uint16_t opcode) {
...
handleCMPM(ESize size, SAddressingMode* src, SAddressingMode* dest, uint16_t data) {
	uint16_t opcode =
	    (uint16_t) (0xB108 | (dest->outer.baseRegister & 7u) << 9 | (src->outer.baseRegister & 7u) | getSizeField(size) << 6);
	sect_OutputConst16(opcode);
	return true;
}
...
handleShift(uint16_t opcode, uint16_t memoryOpcode, ESize size, SAddressingMode* src, SAddressingMode* dest) {
	...
	m68k_AddRegmask(m68k_DestinationUpdatedRegisters(src));
	if (src->mode & (AM_AIND | AM_AINC | AM_ADEC | AM_ADISP | AM_AXDISP | AM_WORD | AM_LONG | AM_AXDISP020 | AM_POSTINDAXD020 |
	                 AM_PREINDAXD020)) {
		if (size != SIZE_WORD) {
			err_Error(MERROR_INSTRUCTION_SIZE);
			return true;
...
handleCAS(ESize sz, SAddressingMode* dc, SAddressingMode* du, uint16_t data) {
	...
	if ((ea.mode & (AM_AIND | AM_AINC | AM_ADEC | AM_ADISP | AM_AXDISP | AM_WORD | AM_LONG | AM_AXDISP020 | AM_PREINDAXD020 |
	                AM_POSTINDAXD020)) == 0) {
		err_Error(ERROR_OPERAND);
		return true;
	}
...
handleMOVEfromSYSREG(ESize sz, SAddressingMode* src, SAddressingMode* dest, uint16_t data) {
	if (src->directRegister == T_68K_REG_USP) {
		...
		EAddrMode allow = AM_DREG | AM_AIND | AM_AINC | AM_ADEC | AM_ADISP | AM_AXDISP | AM_WORD | AM_LONG;
...
handleMOVEM(ESize sz, SAddressingMode* unused1, SAddressingMode* unused2, uint16_t data) {
	...
	if (registerMask != REGLIST_FAIL) {
		...
		if (opt_Current->machineOptions->cpu >= CPUF_68020)
			allowedModes |=
			    AM_AXDISP020 | AM_PREINDAXD020 | AM_POSTINDAXD020 | AM_PCXDISP020 | AM_PREINDPCXD020 | AM_POSTINDPCXD020;
...
SControlRegister g_controlRegister[] = {
    {CPUF_68010 | CPUF_68020 | CPUF_68030 | CPUF_68040 | CPUF_68060 | CPUF_68080, 0x000}, // SFC
    {CPUF_68010 | CPUF_68020 | CPUF_68030 | CPUF_68040 | CPUF_68060 | CPUF_68080, 0x001}, // DFC
    {CPUF_68010 | CPUF_68020 | CPUF_68030 | CPUF_68040 | CPUF_68060,              0x800}, // USP
    {CPUF_68010 | CPUF_68020 | CPUF_68030 | CPUF_68040 | CPUF_68060 | CPUF_68080, 0x801}, // VBR
    {CPUF_68020 | CPUF_68030 | CPUF_68040 | CPUF_68060 | CPUF_68080,              0x002}, // CACR
    {CPUF_68020 | CPUF_68030 | CPUF_68040,                                        0x802}, // CAAR
    {CPUF_68020 | CPUF_68030 | CPUF_68040,                                        0x803}, // MSP
    {CPUF_68020 | CPUF_68030 | CPUF_68040,                                        0x804}, // ISP
    {CPUF_68040 | CPUF_68060 | CPUF_68080,                                        0x003}, // TC
    {CPUF_68040 | CPUF_68060 | CPUF_68080,                                        0x004}, // ITT0
    {CPUF_68040 | CPUF_68060 | CPUF_68080,                                        0x005}, // ITT1
    {CPUF_68040 | CPUF_68060 | CPUF_68080,                                        0x006}, // DTT0
    {CPUF_68040 | CPUF_68060 | CPUF_68080,                                        0x007}, // DTT1
    {CPUF_68040,                                                                  0x805}, // MMUSR
    {CPUF_68040 | CPUF_68060,                                                     0x806}, // URP
    {CPUF_68040 | CPUF_68060,                                                     0x807}, // SRP
    {CPUF_68040,                                                                  0x004}, // IACR0
    {CPUF_68040,                                                                  0x005}, // IACR1
    {CPUF_68040,                                                                  0x006}, // DACR0
    {CPUF_68040,                                                                  0x007}, // DACR1
    {CPUF_68060 | CPUF_68080,                                                     0x008}, // BUSCR
    {CPUF_68060,                                                                  0x807}, // PCR
    {CPUF_68080,                                                                  0x009}, // BRK
    {CPUF_68080,                                                                  0x809}, // CYC
    {CPUF_68080,                                                                  0x80C}, // BPC
    {CPUF_68080,                                                                  0x80D}, // BPW
};
...
handleMOVES(ESize sz, SAddressingMode* src, SAddressingMode* dest, uint16_t data) {
	...
	int allowedModes = AM_DREG | AM_AREG | AM_AIND | AM_AINC | AM_ADEC | AM_ADISP | AM_PCXDISP | AM_WORD | AM_LONG | AM_PCXDISP020 |
...
m68k_CanUseShortMOVEA(ETargetToken targetToken, SAddressingMode* src, SAddressingMode* dest, ESize insSz) {
	return (targetToken == T_68K_MOVE || targetToken == T_68K_MOVEA) && src->mode != AM_AREG && src->directRegisterBank == 1 &&
	       dest->mode == AM_AREG && dest->directRegisterBank == 1 && insSz == SIZE_LONG && m68k_Get68080BankBits(src) == 0;
}

extern bool
m68k_CanUseShortMOVEfromA(ETargetToken targetToken, SAddressingMode* src, SAddressingMode* dest, ESize insSz) {
	return (targetToken == T_68K_MOVE) && src->mode == AM_AREG && src->directRegisterBank == 1 && dest->mode != AM_AREG &&
	       dest->directRegisterBank == 1 && insSz == SIZE_LONG && m68k_Get68080BankBits(dest) == 0;
}
